...
/* static WINDOW textwnd;	/* the new memopad window created and labelled */

/*static char GetSpellText(WINDOW);	/* function to get the pointer to text to be spelt */
...
/*		if(p2 < 0)
		...
		case ID_SPELLCHECK:
		      /*	ErrorMessage("SpellCheck called from MemopadProc ->COMMAND");*/
			break;
		      /*	rtn = DefaultWndProc(wnd, msg, p1, p2);
			return rtn;

	     /*	case ID_CALENDAR:
			/* first gettext pointer */
	     /*		ClearClipboard();	/* clear the Clipboard */
	 /*		CopyToClipboard(textwnd);	/* copy marked text to clipboard */
		     /*	spelltext = GetText(textwnd); /* get full window text */
/*	spelltext = GetText(textwnd); /* get full window text - TOK 12/5/94 */
	 /*		if(ClipboardLength == 0) {
			/*if(spelltext == NULL)		/* if clipboard was empty */
	 /*			spelltext = GetText(textwnd); /* get full window text */
	 /*			}
			else {
...
static int EditorProc(WINDOW wnd,MESSAGE msg,PARAM p1,PARAM p2)
{
	...
	case COMMAND:
			...
			case ID_SPELLCHECK:
 /*------------------------------------------------------------------------*/
				MainSpell(wnd);	/* spell text the window */
